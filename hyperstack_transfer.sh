...
if [ -z "$KEY_PATH" ] || [ -z "$SOURCE_DIR" ] || [ -z "$ZIP_FILE_NAME" ] ||[ -z "$SOURCE_DIR2" ]||[ -z "$SOURCE_DIR3" ]|| [ -z "$DESTINATION_USER" ] || [ -z "$DESTINATION_HOST" ] || [ -z "$DESTINATION_PATH" ]; then
  echo "Missing required environment variables. Please check your .env file."
  exit 1
fi

# Zip the source directory or file
zip -r "$ZIP_FILE_NAME" "$SOURCE_DIR" "$SOURCE_DIR2" "$SOURCE_DIR3"

# Check if the zip command was successful
if [ $? -eq 0 ]; then
  echo "Zipped $SOURCE_DIR1 , $SOURCE_DIR2 and $SOURCE_DIR3 to $ZIP_FILE_NAME successfully."
else
